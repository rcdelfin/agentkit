...
def triage_tasks(tasks):
    ...
    for t in tasks:
        ...
        updated_ts = t.get("date_updated", 0)
        ...
        updated_dt = to_datetime(updated_ts)
        due_dt = to_datetime(due_ts)
...
def generate_report(
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Generated by AgentKit ClickUp Triage*")
...
def main():
    ...
    if assignee:
        try:
            ...
            parser.error("--assignee must be a numeric ClickUp user ID")
        before = len(tasks)
        tasks = [
            t
            for t in tasks
