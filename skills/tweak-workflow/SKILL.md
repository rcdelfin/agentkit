...
1. **Understand the request.** Read the repository guidance and relevant code.
   Classify the change:
...
3. **Route and plan.** Use `skill-orchestration` to load only skills required by
   ...
   design, and tasks before implementation. Use `scrutinize` before implementation
   when the change is risky, public-contract, security-sensitive, cross-boundary,
